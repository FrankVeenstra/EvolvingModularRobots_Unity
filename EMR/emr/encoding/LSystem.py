...
class GraphGrammar:
    def __init__(self, controller_reference, config, number_of_symbols : int = 5, fitness = -1.0) -> None:
        from emr.config import config_handler as ch
        module_options = ch.modules_to_use()
    ...
    def random_rules(self, controller_reference, config, number_of_symbols = 5) -> None:
        from emr.config import config_handler as ch
        module_options = ch.modules_to_use()
        module_list = list(module_options)
    ...
    def manual_rules(self, controller_reference, config, number_of_symbols : int = 5) -> None:
        ...
        number_of_module_types_to_use = len(module_options)

        # stores the string references (for random.choice(options))
        self.options = []
        self.axiom = module_options[0].name
        for i in range(number_of_symbols):
            moduleNr = i % number_of_module_types_to_use
            module = module_options[moduleNr]
            t = module.name
