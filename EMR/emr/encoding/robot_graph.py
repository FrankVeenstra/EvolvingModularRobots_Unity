...
def step_update_controllers(controller_list, sensory_input, actions, delta_time):
	action_index = 0
	for i,controller in enumerate(controller_list):
		action = controller.step([sensory_input[0][i]], [actions[0][i]], delta_time)
...
class Blueprint:
	...
	@staticmethod
	def random(n_nodes: int, ControllerReference):
		"""
		...
		random_blueprint.nodes.update({"root":Node("root",type = 'EmergeModuleUnpacked')})
		random_blueprint.controllers.update({"root":ControllerReference.random('root')})
		for i in range(n_nodes):
			node_hash = str(uuid.uuid4())
			parent_node = random.choice(list(random_blueprint.nodes.keys()))
			# TODO change to the number of connections available to the module
			connection_site = str(random.randint(0,2))
			...
			if (can_create_new_node):
				...
				random_blueprint.nodes.update({node_hash:Node(node_hash,parent=parent_node, connection_site =connection_site, type = 'EmergeModuleUnpacked',angle=angle)})
				random_blueprint.controllers.update({node_hash:ControllerReference.random(node_hash)})
