...
class PhaseCoupledOscillator(AbstractController):
	...
	def step(self, output_buffer, input_buffer, delta_time : float):
		...
		for i in range(len(output_buffer)):
			#if (i == 0):
				output_buffer[i] = self.desired_joint_angle
				# This is the output buffer of the joint and will set this value as an action
			#else:
			#	output_buffer[i] = 0
			#	pass
				# TODO
				#output_buffer[i] = desired_phase_difference;
