...
def visualize(graph, ax = None, vg = None, plot_nerves = True, plot_controllers = True):
    ...
    if (vg == None):
        ...
        while (len(queue) > 0):
            ...
            for c in children:
                ...
                ascending_color = np.array(graph.innervation[c].parents[node].value)
                descending_color = np.array(graph.innervation[node].children[c].value)
    ...
    for n in vg.vnodes:
        ...
        color = np.array(matplotlib.colormaps[cmap](0.5+(0.5*node_color_value)))
        ...
        if (node.parent):
            ...
            if (plot_nerves):
                ...
                # descending nerve
                dnc = np.array(node.descending_color)
                ...
                anc = np.array(parent.ascending_color)
...
if __name__ == "__main__":
    from encoding import graph_grammar
    for i in range(10):
        l = graph_grammar.GraphGrammar(3)
        for i in range(4):
            l.iterate()
