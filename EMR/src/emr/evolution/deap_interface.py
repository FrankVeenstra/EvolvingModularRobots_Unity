...
#class Individual:
#    def __init__(self, controller_reference, modules_to_use, fitness=0.0):
...
class EvolutionaryAlgorithm:
    def __init__(self, evaluation_function, config, individual_reference, controller_reference, modules_to_use, n_cores : int = 0, run_number : int = 0):
        ...
        if (n_cores == 0):
            self.n_cores = int(config['ea']['n_cores'])
        else:
        ...
        toolbox.register("evaluate",evaluation_function, 
               ...
               n_steps = number_of_evaluation_steps, scene_number_to_load = int(config['experiment']['scene_number']), 
                no_graphics = int(config['visualization']['headless']),editor_mode =int(config['environment']['run_in_editor_mode']))
    ...
    def initialize(self,config):
        ...
        print("Initializing EA, evaluating initial population")
        if (self.n_cores == 1):
            self.evaluate_population(self.pop)
            self.toolbox.register("close", evaluation.close_env)  
        else:
    ...
    def evaluate_population(self,offspring):
        if (self.n_cores == 1):
            ...
            # Fitness stored in object used in thread not overriden as it is stored in a thread, manual override here:
    ...
    def load_best(self, config):
        ...
        for i in range(1):
            ...
            fit = self.evaluation_function(last_ind, executable_path, scene_number_to_load = int(config['experiment']['scene_number']),n_steps=number_of_evaluation_steps)
            print(f"fitness : {fit}")
